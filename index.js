 ...
 */
function createCacheKey(thing) {
  return createHashFromString(JSON.stringify(thing));
}
 ...
 */
function writeToCache(cacheKey, result, cacheLocation) {
  return new Promise((resolve, reject) => {
    mkdirp(cacheLocation, () => {
      const location = path.join(cacheLocation, `${cacheKey}.json`);
      fs.writeFile(location, JSON.stringify(result), (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
